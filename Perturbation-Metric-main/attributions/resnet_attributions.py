...
class ResnetAttributions:
	def __init__(self):
		...
		self.gc = GradCAM(model=self.model, target_layers=[self.model.layer4], use_cuda=True)
	...
	def gen_resnet_attributions(self, image_file, output_path):
		transform_normalize_resnet = transforms.Normalize(mean=[0.485, 0.456, 0.406],std=[0.229, 0.224, 0.225])
		...
		normalized_images = transform_normalize_resnet(float_images).cuda()
